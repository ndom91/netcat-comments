...
>
type DiscussionAction = Extract<
  keyof typeof Actions,
  "CREATE_REPLY" | "CREATE_DISCUSSION" | "GET_DISCUSSION" | "LIST_DISCUSSIONS"
>
...
const getMessageType = (action: keyof typeof Actions) => {
  ...
  const discussionActions = [
    Actions.CREATE_REPLY,
    Actions.CREATE_DISCUSSION,
    Actions.GET_DISCUSSION,
    Actions.LIST_DISCUSSIONS,
  ]
...
export const parseMessage = (userId: string, bodyString: string): Message => {
  const rawBody = bodyString.split("|").reduce(
    (body, segment, i) => {
